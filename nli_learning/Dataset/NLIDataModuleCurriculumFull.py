...
from nli_learning.Dataset.NLIDatasetCurriculumFull import NLIDatasetCurriculumFull

class NLIDataModuleCurriculumFull(pl.LightningDataModule):
    ...
    def __init__(self, data_source_folder, dataset_name, batch_size = 64, train_stage = 0, transform = None):
        ...
        self.train_source_data = os.path.join(self.data_source_folder, 'train_curriculum_full.json')
    ...
    def train_dataloader(self):
        return DataLoader(NLIDatasetCurriculumFull(self.train_source_data, transform = self.transform, train_stage= self.train_stage), batch_size=self.batch_size, shuffle=False)
