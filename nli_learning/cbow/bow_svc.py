...
def transform_dataset_to_bow(dataset):
    ...
    for item in dataset:
        ...
        for sentence_1,sentence_2,label in zip(all_sentence_1,all_sentence_2,all_labels):
...
def train_svc(train_input,train_labels,validation_input,validation_labels,test_inputs,test_labels):
    '''
    param_grid = {
        'C': [0.1, 0.5, 1, 2, 50],
    ...
    '''
    param_grid_svc = {'C': [0.5], 'class_weight': ['balanced'], 'max_iter': [2500], 'tol': [1e-05], 'verbose': [1]}
...
def train_xgb(train_input,train_labels,validation_input,validation_labels,test_inputs,test_labels):
    ...
    param_grid_xgb = {'colsample_bytree': [1.0], 'gamma': [0.5], 'max_depth': [5], 'min_child_weight': [1], 'subsample': [1.0]}
