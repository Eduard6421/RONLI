...
class BertOversampleClassifier(pl.LightningModule):
    def __init__(self, stats_folder: str):
        ...
        self.precision_each = Precision(
            task="multiclass", num_classes=self.num_classes, average=None
        )

        self.recall_micro = Recall(
            task="multiclass", num_classes=self.num_classes, average="micro"
        )
        self.recall_macro = Recall(
            task="multiclass", num_classes=self.num_classes, average="macro"
        )
        self.recall_each = Recall(
            task="multiclass", num_classes=self.num_classes, average=None
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(len(guid)):
            items.append(
                {
                    "guid": guid[i],
                    f"logits_epoch_{self.current_epoch}": x[i]
                    .cpu()
                    .detach()
                    .numpy()
                    .tolist(),
    ...
    def on_train_epoch_end(self):
        with open(
            f"{self.stats_folder}/dynamics_epoch_{self.current_epoch}.jsonl",
            "w",
            encoding="UTF-8",
        ) as f:
    ...
    def test_step(self, batch, batch_idx):
        ...
        for i in range(self.num_classes):
            ...
            self.log(
                f"f1_{i}", class_f1[i], on_step=False, on_epoch=True, prog_bar=True
            )
        ...
        self.log(
            "test_accuracy", self.accuracy, on_step=False, on_epoch=True, prog_bar=True
        )
        return loss
