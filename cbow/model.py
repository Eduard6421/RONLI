...
from nli_learning.cbow.bow_svc import train_svc, train_xgb, transform_dataset_to_bow
from sklearn.preprocessing import MinMaxScaler

def train_pipeline(args):
    dataset_path = "dataset/datasets"
    dataset = NLIDataModule(data_source_folder=dataset_path,dataset_name='bow_dataset', batch_size=64)
    ...
    if(args.subtype == 'svc'):
        print('Train SVC')
        train_svc(train_inputs,train_labels,val_inputs,val_labels,test_inputs,test_labels)
    elif(args.subtype == 'xgb'):
        print('Train XGB')
        train_xgb(train_inputs,train_labels,val_inputs,val_labels,test_inputs,test_labels)
    else:
