...
def consider_ascending_order(filtering_metric: str) -> bool:
  ...
  if filtering_metric == "variability":
    ...
    return True
  elif filtering_metric == "new_score":
    return True
  else:
...
def write_filtered_data(args, train_dy_metrics):
  ...
  sorted_scores = train_dy_metrics.copy()
  sorted_scores['relative_variance'] = sorted_scores['variability'].apply(lambda x: 1 - x if x < 0.5 else x)
  sorted_scores['new_score'] = (1 - sorted_scores['correctness'])  + sorted_scores['relative_variance']

  # Sort by selection.
  sorted_scores = sorted_scores.sort_values(by=[args.metric],
                                               ascending=is_ascending)
  

  print(sorted_scores.columns)
  ...
  # read date from the utf-8 file original_train_file

  train_numeric = []
  with open(original_train_file, "r", encoding="utf-8") as infile:
    train_numeric = json.load(infile)

  # make a map of every guid to its item from the original train_numeric
  guid_to_item = {}
  for item in train_numeric:
    guid_to_item[item["guid"]] = item


  for fraction in [0.01, 0.05, 0.10, 0.1667, 0.25, 0.3319, 0.50, 0.75, 1]:
    ...
    with open(os.path.join(outdir, f"train.json"), "w", encoding="UTF-8") as outfile:
      full_output = []    
      ...
      for idx in selection_iterator:
        ...
        record = guid_to_item[selected_id]
        full_output.append(record)
      # write the utf-8 object to fiel
      json.dump(full_output, outfile, indent=4, sort_keys=True, ensure_ascii=False)
...
def plot_data_map(dataframe: pd.DataFrame,
                  ...
                  max_instances_to_plot = 60000):
    

    font_size = 22

    # Set style.
    sns.set(style='whitegrid',  font='Georgia', context='paper')
    ...
    # Make a copy of the dataframe and sort it by the main metric.
    #dataframe_copy = dataframe.copy()
    #dataframe_copy.sort_values(by=[main_metric], inplace=True)
    ...
    if not show_hist:
        plot.legend(title="correctness",ncol=3, bbox_to_anchor=[0.175, 0.5], loc='right', fontsize=font_size-4)
        legend = plot.legend(title='Correctness')
        legend.get_title().set_fontsize(font_size)
    else:
        plot.legend(title="correctness",fancybox=True, shadow=True,  ncol=3, fontsize=font_size-4)
    plot.set_xlabel('variability', fontsize=font_size)
    plot.set_ylabel('confidence', fontsize=font_size)
    ...
    if show_hist:
        ...
        plott0[0].set_title('')
        plott0[0].set_xlabel('confidence', fontsize=font_size)
        plott0[0].set_ylabel('density', fontsize=font_size)

        plott1 = dataframe.hist(column=['variability'], ax=ax2, color='teal')
        plott1[0].set_title('')
        plott1[0].set_xlabel('variability', fontsize=font_size)
        plott1[0].set_ylabel('density', fontsize=font_size)

        df_sorted = dataframe.sort_values('correct.')

        plot2 = sns.countplot(x="correct.", data=df_sorted, ax=ax3, color='#86bf91')
        ...
        filtered_ticks = remove_indexes(plot2.get_xticks(), [1, 2, 4 ,5 , 7 ,8, 9])
        filteredxticks_labels = remove_indexes(plot2.get_xticklabels(),[1,2,4,5,7,8,9])

        plot2.set_xticks(filtered_ticks)
        plot2.set_xticklabels(filteredxticks_labels, fontsize=font_size)

        ax3.xaxis.grid(True) # Show the vertical gridlines

        plot2.set_title('')
        plot2.set_xlabel('correctness', fontsize=font_size)
        plot2.set_ylabel('density', fontsize=font_size)
